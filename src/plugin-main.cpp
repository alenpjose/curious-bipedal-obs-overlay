...
void update_clock_children(Overlay *overlay, uint64_t now)
{
	...
	update_text_source(overlay->date_text, overlay->show_date ? date_string : "", 17, false, overlay->opacity,
			   rgba(174, 183, 191, 255), 190, 36, "right");
}
...
void overlay_render(void *data, gs_effect_t *)
{
	...
	render_source(overlay->log_text, text_x, session_y + 98.0f * scale, scale, scale);
	...
	if (show_date || show_time || show_timer) {
		...
		render_source(overlay->time_text, telemetry_x + 48.0f * scale, telemetry_y + 75.0f * scale, scale, scale);
		render_source(overlay->date_text, telemetry_x + 222.0f * scale, telemetry_y + 75.0f * scale, scale, scale);
	}
}
